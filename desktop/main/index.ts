...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(async () => {
    ...
    mainWindow = new BrowserWindow({ width: bounds.width || 1280, height: bounds.height || 820, x: bounds.x, y: bounds.y, minWidth: 980, minHeight: 680, show: false, webPreferences: { preload: join(__dirname, "../preload/index.cjs"), contextIsolation: true, nodeIntegration: false, sandbox: true, webSecurity: true, devTools: !app.isPackaged } });
    mainWindow.once("ready-to-show", () => mainWindow?.show());
    mainWindow.on("close", () => { if (mainWindow) sqlite.prepare("INSERT INTO desktop_preferences(key,value_json)VALUES('window',?) ON CONFLICT(key) DO UPDATE SET value_json=excluded.value_json,updated_at=CURRENT_TIMESTAMP").run(JSON.stringify(mainWindow.getBounds())); });
    ...
    Menu.setApplicationMenu(Menu.buildFromTemplate([{ label: "文件", submenu: [{ label: "导出备份", click: () => void exportBackup() }, { label: "导入备份", click: () => void importBackup() }, { type: "separator" }, { role: "quit", label: "退出" }] }, { label: "数据", submenu: [{ label: "打开数据目录", click: () => void shell.openPath(dirname(databasePath(app.getPath("userData")))) }] }, { label: "帮助", submenu: [{ label: "关于 AI Trip Planner", click: () => void dialog.showMessageBox({ type: "info", title: "关于", message: `AI Trip Planner ${app.getVersion()}`, detail: "Apache-2.0" }) }] }]));
