...
const headersFor = (request: Request) => Object.fromEntries(["oai-authenticated-user-email", "oai-authenticated-user-full-name"].map(name => [name, request.headers.get(name) || ""]).filter(([, value]) => value));
