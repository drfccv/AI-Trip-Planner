...
<p align="center">
	<img src="desktop/assets/icon.svg" alt="旅迹桌面图标" width="112" />
</p>

旅迹是一款由 AI 辅助规划行程的旅行工作台，支持逐日安排、对话式修改、地图与天气、MCP 旅行工具、版本恢复和日历导出。

- **GitHub 仓库**：[drfccv/lvji-travel](https://github.com/drfccv/lvji-travel)
- **桌面安装包**：[Releases](https://github.com/drfccv/lvji-travel/releases)
- **Web 版**：`main` 分支
- **桌面版**：`drfccv/electron-local` 分支

项目采用 [Apache-2.0 License](./LICENSE)。

## 一眼看懂

- 面向旅行计划的 AI 工作台，支持逐日行程编排与对话式调整
- 桌面版将应用、SQLite 数据库和运行环境一并打包，开箱即可用
- 同时支持 Web 端与桌面端，便于按场景切换使用

## 桌面版

桌面版将应用、SQLite 数据库和运行环境一并打包，安装后不需要额外安装 Node.js、pnpm、PostgreSQL 或其他开发工具。
...
1. 打开 [GitHub Releases](https://github.com/drfccv/lvji-travel/releases)
2. 下载对应平台的最新安装包
3. 运行安装包并按提示完成安装

当前自动构建 Windows、macOS 和 Linux 安装包。首次使用时，可在应用设置中配置 AI 服务、地图、天气和 MCP 凭证。

> 未配置代码签名证书时，部分平台可能显示未知发布者提示。请仅从本仓库 Releases 页面下载安装包。
...
AI 与 MCP 凭证通过 Electron `safeStorage` 使用当前系统账户的系统能力加密。Renderer 只能读取配置状态和脱敏值，无法取得完整密钥。普通 JSON 行程备份不包含 AI 或 MCP 凭证，换机后需要重新配置。
